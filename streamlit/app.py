...
Streamlit in Snowflake app comparing three approaches to transforming
complex nested JSON from OpenAI APIs.
"""
...
st.set_page_config(page_title="OpenAI Data Engineering", layout="wide")
st.title("OpenAI Data Engineering in Snowflake")
st.caption("Three approaches to transforming complex nested API response data")

approach = st.sidebar.radio(
    "Select Approach",
    [
        "1 - Schema-on-Read (Views)",
        "2 - Medallion (Dynamic Tables)",
        "3 - Cortex Enrichment",
        "Raw Data Explorer",
    ],
)
...
# ---------------------------------------------------------------------------
if approach == "1 - Schema-on-Read (Views)":
    st.header("Approach 1: Schema-on-Read with FLATTEN + Views")
    st.markdown(
...
# ---------------------------------------------------------------------------
elif approach == "2 - Medallion (Dynamic Tables)":
    st.header("Approach 2: Medallion Architecture with Dynamic Tables")
    st.markdown(
    ...
    with tab_gold:
        ...
        if gold_obj == "DT_DAILY_TOKEN_SUMMARY":
...
# ---------------------------------------------------------------------------
elif approach == "3 - Cortex Enrichment":
    st.header("Approach 3: Cortex AI Enrichment Pipeline")
    st.markdown(
    ...
    st.info(
        "Cortex dynamic tables (DT_ENRICHED_COMPLETIONS, DT_BATCH_ENRICHED, DT_PII_SCAN) "
        "are commented out in deploy_all.sql to avoid unexpected credit usage. "
        "Deploy from sql/03_transformations/03_approach3_cortex.sql to enable."
    )
    ...
    with tab_enrich:
        try:
            ...
            st.warning("DT_ENRICHED_COMPLETIONS not deployed. Run Approach 3 SQL to enable.")
    ...
    with tab_batch_qa:
        try:
            ...
            st.warning("DT_BATCH_ENRICHED not deployed. Run Approach 3 SQL to enable.")
    ...
    with tab_pii:
        try:
            ...
            st.warning("DT_PII_SCAN not deployed. Run Approach 3 SQL to enable.")
    ...
    with tab_dash:
        try:
            ...
            st.warning("V_ENRICHMENT_DASHBOARD not deployed. Run Approach 3 SQL to enable.")
